...
- The metrics are passed through the graph state and rendered on the results page as a visual badge:
  - 🟢 **High Quality (Green)**: $\ge$ 8 chunks retrieved, ensuring deep filing coverage.
  - 🟡 **Partial Quality (Amber)**: 3 to 7 chunks retrieved, indicating moderate context.
  - ⚪ **Limited Quality (Gray)**: < 3 chunks, warning the user that analysis relied primarily on web data.
...
- **Exponential Backoff with Jitter**: If an API returns a `429 Too Many Requests` or `5xx Server Error`, the system waits and retries. The delay increases exponentially (e.g., 200ms $\rightarrow$ 400ms $\rightarrow$ 800ms) with a randomized "jitter" offset to prevent synchronized retry storms.
...
- The results page includes a dedicated **Trend** tab that displays:
  - A visual timeline representing recommendation shifts (e.g., `INVEST` $\rightarrow$ `PASS`).
