...
1. **IP Whitelisting (MongoDB Atlas)**:
   ...
   - Go to your MongoDB Atlas Dashboard $\rightarrow$ **Network Access** $\rightarrow$ **IP Access List**.
   - Click **Add IP Address** and add your current IP address (or select **Allow Access From Anywhere (0.0.0.0/0)** for testing).
...
- **StockSage Resilience**:
  - You do not need to restart the application. StockSage features a built-in exponential backoff retry system (`lib/utils/rateLimiter.ts`).
...
- **StockSage Compliance**:
  - StockSage includes a compliant User-Agent in [lib/tools/edgar.ts](file:///c:/Users/ankus/Desktop/Assigment%20Task/investment-agent/lib/tools/edgar.ts):
...
### Symptom
- Console prints: `⚠️ ChromaDB unavailable — using MongoDB RAG fallback. Details: Error: ...`
...
- **Cloud Database (Chroma Cloud) Setup**:
  ...
  - If you receive authentication errors, test the API Key directly using curl/Postman to ensure it is still active and valid.
